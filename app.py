...
def generate_pptx(df, program_studi, nama_mata_kuliah, project_title):
  ...
  for index, row in df.iterrows():
    ...
    if row.get('Saran dan Info'):
        saran_box = slide.shapes.add_textbox(Inches(0.8), Inches(6.9), Inches(11.7), Inches(0.5))
...
if not st.session_state.logged_in:
  ...
  with col2:
    if not check_db_connection():
      st.error(
          "⚠️ **Database MySQL Tidak Aktif!** Pastikan XAMPP/MySQL Anda sudah"
          " dinyalakan."
      )
          ...
          try:
            ...
            if user:
              ...
              st.session_state.storyboard_df = pd.DataFrame(columns=[
                  "Judul Scene", "Visualisasi", "Instruksi untuk Visual", 
                  "Animasi", "On-Screen Text", "Voice Over Text", 
                  "Backsound", "Durasi", "Saran dan Info"
              ])
...
# ==========================================
  ...
  with st.sidebar:
    ...
    if check_db_connection():
      st.caption("🟢 MySQL Database: Connected")
    else:
      st.caption("🔴 MySQL Database: Disconnected")
...
# ==========================================
  ...
  st.markdown(
      ...
      " Truth Dataset menggunakan Ollama</div>",
      unsafe_allow_html=True,
  )
  ...
  # ==========================================
  # KOLOM 1: INPUT & SETTINGS (DISET KOSONG)
  # ==========================================
  with col_left:
    st.subheader("Project Input & Ground Truth Settings")

    local_model_name = st.selectbox(
        "Pilih Model Ollama Lokal", ["llama3", "mistral", "phi3"]
    )

    program_studi = st.text_input(
        "Program Studi", value=""
    )
    nama_mata_kuliah = st.text_input(
        "Nama Mata Kuliah", value=""
    )
    ...
    target_audience = st.selectbox(
        "Target Audience",
        ["-- Pilih Target Audience --", "Mahasiswa S1/S2 (Advanced)", "Beginner Students", "General"],
    )
    visual_style = st.selectbox(
        "Visual Style",
        ["-- Pilih Visual Style --", "Animasi Diagram Blok & Rumus Matematis", "Biology Lab", "Cinematic"],
    )
  ...
  with col_middle:
    ...
    if generate_btn:
      if not program_studi or not nama_mata_kuliah or not learning_objectives:
        st.error("⚠️ Harap isi Program Studi, Nama Mata Kuliah, dan Learning Objectives terlebih dahulu!")
        ...
        with st.spinner(
            "🔄 Menjalankan Local RAG dengan Standar Ground Truth..."
        ):
          try:
            ...
            prompt = (
                "SYSTEM: Anda adalah API generator JSON murni. Tugas Anda adalah mengembalikan HANYA valid JSON array tanpa teks pengantar, tanpa penjelasan, dan tanpa markdown block.\n\n"
                "Berdasarkan dokumen kurikulum referensi berikut, buatlah rancangan storyboard pembelajaran yang komprehensif:\n\n"
            ...
            scenes_json = parse_llm_json_response(response)
            ...
            if scenes_json and isinstance(scenes_json, list):
              ...
              if check_db_connection():
                ...
                query_main = """
                                    INSERT INTO storyboards (username, program_studi, nama_mata_kuliah, project_name, learning_objectives, target_audience, visual_style, rps_filename) 
                ...
                query_scene = """
                                    INSERT INTO storyboard_scenes (storyboard_id, judul_scene, visualisasi, instruksi_visual, animasi, on_screen_text, voice_over_text, backsound, durasi, saran_info) 
              ...
              st.error(
                  "Gagal memproses format JSON dari model lokal. Pastikan"
                  " Ollama sedang berjalan."
              )

          except Exception as e:
            st.error(
                f"Terjadi kesalahan pada model lokal (Ollama): {e}\nPastikan"
                " aplikasi Ollama aktif di komputer Anda."
            )
    ...
    if st.session_state.rag_contexts:
      ...
      st.info(
          "Belum ada konteks aktif. Upload dokumen RPS PDF dan klik 'Generate Storyboard'."
      )
  ...
  with col_right:
    ...
    st.markdown(
        f"📋 **Prodi:** {program_studi if program_studi else '-'} | **Mata Kuliah:** {nama_mata_kuliah if nama_mata_kuliah else '-'} | 📄 **RPS:** `{st.session_state.uploaded_rps_name}`"
    )
    st.divider()
    ...
    if st.session_state.storyboard_df.empty:
      st.info("Belum ada scene yang dimuat atau digenerate. Klik '➕ Add New Scene' atau 'Generate Storyboard' untuk memulai.")
    else:
    ...
    if st.session_state.storyboard_df.empty:
      ...
      pptx_data = generate_pptx(st.session_state.storyboard_df, program_studi, nama_mata_kuliah, project_name)
      st.download_button(
          label="📥 Export to PPT",
          data=pptx_data,
          file_name=f"{project_name.replace(' ', '_')}_Storyboard.pptx" if project_name else "Storyboard.pptx",
          mime=(
    ...
    # ==========================================
    # TABEL DAFTAR RIWAYAT PROYEK USER (DENGAN INFORMASI RPS)
    # ==========================================
    st.markdown("---")
    with st.expander("📂 Riwayat Proyek Storyboard Anda", expanded=False):
      if check_db_connection():
        try:
          conn = get_db_connection()
          query_history = "SELECT id, program_studi, nama_mata_kuliah, project_name, rps_filename, created_at FROM storyboards WHERE username = %s ORDER BY id DESC"
          df_history = pd.read_sql(query_history, conn, params=(st.session_state.username,))
          conn.close()
          ...
          if not df_history.empty:
            ...
            if selected_rows:
              ...
              selected_proj_name = df_history.iloc[selected_index]["project_name"]
              selected_rps = df_history.iloc[selected_index]["rps_filename"]

              if st.button(f"📂 Muat Proyek '{selected_proj_name}' (ID: {selected_proj_id}) ke Editor"):
                conn = get_db_connection()
                query_load_scenes = """
                ...
                """
                df_scenes_loaded = pd.read_sql(query_load_scenes, conn, params=(selected_proj_id,))
                conn.close()

                if not df_scenes_loaded.empty:
                  df_scenes_loaded.rename(columns={
                  ...
                  st.success(f"✨ Proyek '{selected_proj_name}' (Ref RPS: {selected_rps}) berhasil dimuat ke editor!")
                  st.rerun()
                else:
                  st.warning("⚠️ Tidak ada data scene yang ditemukan untuk proyek ini.")
          else:
